...
input_data = """1-13 r: gqdrspndrpsrjfjx
...
def parse_input(line):
    ...
    rng, char = check.split(" ")
    mina, maxa = [int(e) for e in rng.split("-")]
    mina -= 1
    maxa -= 1
    return mina, maxa, char, password

num_passwords_ok = 0
for line in input_data:
    mina, maxa, char, password = parse_input(line)
    if password[mina] != char and password[maxa] != char:
        continue
    if password[mina] == char and password[maxa] == char:
        continue
    num_passwords_ok += 1

print(num_passwords_ok)
